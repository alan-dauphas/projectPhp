...
//function for administrationView
  public function readAllComments(){
    ...
    $result = $req->fetchAll();
      foreach ($result as $row) { // je parcours toutes mes lignes de ma base de données.
        ...
        $comment->create($row['id'],$row['author'],$row['comment'],$row['reporting'],$row['comment_date_fr']);
  ...
  public function readAllCommentsSignaled(){
    $db = $this->dbConnect();
    $req = $db->query('SELECT id, author, comment, reporting, DATE_FORMAT(comment_date, \'%d/%m/%Y à %Hh%imin%ss\') AS comment_date_fr FROM comments WHERE reporting = "1"');
